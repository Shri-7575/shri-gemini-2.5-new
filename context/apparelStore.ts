...
export const createApparelSlice: StudioStoreSlice<ApparelSlice> = (set, get) => ({
  ...
  generateAIModel: async (prompt) => {
    ...
    try {
      ...
      const imageB64 = await withRetry(() => geminiService.generateImage(finalPrompt, '3:4'));
      if (imageB64) {
  ...
  addApparelItem: async (base64) => {
      ...
      // Run AI tasks in parallel immediately
      ...
      // Handle analysis
      analysisPromise.then(({ description, category }) => {
          set(state => ({
      ...
      try {
          ...
          set({ error: "Art Director was unable to generate concepts for this item." });
      } finally {
