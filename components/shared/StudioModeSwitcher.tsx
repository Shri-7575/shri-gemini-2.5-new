...
import { Shirt, Package, Palette, ImageUp, MoreHorizontal, Images, MessageSquare, Stamp, Zap, Edit3, Share2 } from 'lucide-react';
...
export const StudioModeSwitcher: React.FC = () => {
    ...
    const isMoreMenuActive = studioMode === 'design' || studioMode === 'reimagine' || studioMode === 'product-scene-generator' || studioMode === 'photoshoot' || studioMode === 'chat' || studioMode === 'logo-studio' || studioMode === 'lite-studio' || studioMode === 'edit-studio' || studioMode === 'social-studio';
        ...
        <div ref={dropdownRef} className="relative flex-shrink-0 bg-zinc-900 p-1 rounded-full flex items-center gap-1 border border-zinc-800 shadow-inner-soft">
            ...
            {isDropdownOpen && (
                <div className="absolute top-full right-0 mt-2 w-48 bg-zinc-900 border border-white/10 rounded-lg shadow-2xl z-50 p-2 animate-fade-in duration-150">
                    <button
                        onClick={() => handleModeChange('social-studio')}
                        className={`w-full flex items-center gap-3 p-2 text-sm font-medium rounded-md transition-colors
                        ${studioMode === 'social-studio' ? 'bg-zinc-700 text-white' : 'text-zinc-300 hover:bg-zinc-800 hover:text-zinc-100'}`}
                    >
                        <Share2 size={16} className="text-violet-400" />
                        <span>Social Post Studio</span>
                    </button>
                     <button
                        onClick={() => handleModeChange('lite-studio')}
                        className={`w-full flex items-center gap-3 p-2 text-sm font-medium rounded-md transition-colors
                        ${studioMode === 'lite-studio' ? 'bg-zinc-700 text-white' : 'text-zinc-300 hover:bg-zinc-800 hover:text-zinc-100'}`}
                    >
                        <Zap size={16} className="text-yellow-400" />
                        <span>Lite Studio</span>
                    </button>
                    <button
                        onClick={() => handleModeChange('edit-studio')}
                        className={`w-full flex items-center gap-3 p-2 text-sm font-medium rounded-md transition-colors
                        ${studioMode === 'edit-studio' ? 'bg-zinc-700 text-white' : 'text-zinc-300 hover:bg-zinc-800 hover:text-zinc-100'}`}
                    >
