...
import { ArrowLeft, Bot, ImageIcon, Image as ImageIconSolid, Loader2, Clipboard, Check, Send, ImageUp, X, Plus, Trash2, Download } from 'lucide-react';
import { useStudio } from '../../context/StudioContext';
import { TabButton } from '../shared/TabButton';
import type { ChatMessage, ChatSession } from '../../types';

// FIX: Added React.FC type to ChatImage to support 'key' prop when mapping elements.
const ChatImage: React.FC<{ src: string; alt: string }> = ({ src, alt }) => {
    ...
    return (
        <div className="relative group max-w-xs mb-2">
            <img src={src} className="rounded-lg w-full border border-white/10 shadow-md" alt={alt} />
            <button 
                onClick={handleDownload}
                className="absolute top-2 right-2 p-2 bg-black/60 hover:bg-black/80 rounded-lg text-white opacity-0 group-hover:opacity-100 transition-all shadow-lg backdrop-blur-sm"
...
const CreativeDirectorChatPanel: React.FC = () => {
    ...
    return (
        <div className="flex flex-col h-full max-w-3xl mx-auto w-full">
            <div className="flex-grow overflow-y-auto pr-2 space-y-6 scroll-smooth custom-scrollbar">
                {activeChat.history.map((msg, index) => (
                    <div key={index} className={`flex items-start gap-3 ${msg.role === 'user' ? 'justify-end' : ''}`}>
                        ...
                        <div className={`p-3 rounded-xl max-w-lg ${msg.role === 'user' ? 'bg-zinc-700 text-white' : 'bg-zinc-800 text-zinc-300 shadow-sm border border-white/5'}`}>
                            {msg.parts.map((part, i) => {
                                if ('inlineData' in part && part.inlineData) {
                                    const { mimeType, data } = part.inlineData;
                                    return <ChatImage key={i} src={`data:${mimeType};base64,${data}`} alt="Chat output" />
                                }
                ...
                {isChatting && (
                     <div className="flex items-start gap-3">
                        ...
                        <div className="p-3 rounded-xl bg-zinc-800 text-zinc-300 shadow-sm border border-white/5">
            ...
            <div className="flex-shrink-0 pt-4 mt-4 border-t border-zinc-800">
                 {imageToSend && (
                    <div className="relative w-24 h-24 mb-3 p-1 bg-zinc-800 rounded-lg border border-zinc-700 shadow-inner">
                        <img src={imageToSend} alt="Preview" className="w-full h-full object-cover rounded"/>
                        <button onClick={() => setImageToSend(null)} className="absolute -top-2 -right-2 p-1 bg-zinc-700 rounded-full text-white hover:bg-red-500 transition-colors shadow-lg">
                            <X size={14} />
                        </button>
                    </div>
                )}
                <div className="relative">
                    ...
                    <button onClick={handleAttachmentClick} className="absolute top-1/2 left-2 -translate-y-1/2 p-2 rounded-full text-zinc-400 hover:text-white transition-colors" title="Attach Image">
                        <ImageUp size={20} />
                    </button>
                    <input
                        type="text"
                        ...
                        className="w-full bg-zinc-800 border border-zinc-700 rounded-full p-3 pl-12 pr-12 text-sm text-zinc-200 focus:ring-2 focus:ring-violet-500 focus:border-violet-500 transition-all shadow-inner-soft"
                    />
                    <button onClick={handleSend} disabled={isChatting || (!input.trim() && !imageToSend)} className="absolute top-1/2 right-2 -translate-y-1/2 p-2 rounded-full bg-violet-600 hover:bg-violet-500 text-white disabled:bg-zinc-600 transition-all shadow-md active:scale-95">
                        <Send size={16} />
                    </button>
                </div>
            </div>
    ...
    return (
        <div className="flex flex-col flex-grow min-h-0">
            <div className="flex items-center justify-between mb-2 px-1">
                <h3 className="font-bold text-zinc-400 text-xs uppercase tracking-widest">History</h3>
                 <button onClick={startNewChat} className="flex items-center gap-2 px-2 py-1 rounded-md text-xs font-semibold transition-all bg-zinc-800 text-zinc-300 hover:bg-zinc-700 hover:text-white border border-white/5 shadow-sm active:scale-95">
                    <Plus size={14} /> New Chat
                </button>
            </div>
            <div className="flex-grow overflow-y-auto -mx-2 pr-1 space-y-1 custom-scrollbar">
                {sortedChats.map(chat => (
                    <div key={chat.id} className="group flex items-center justify-between rounded-md pr-1 hover:bg-zinc-800/50 transition-colors">
                        <button 
                            onClick={() => setActiveChat(chat.id)} 
                            className={`flex-1 text-left px-3 py-2 rounded-md text-sm transition-colors truncate ${activeChatId === chat.id ? 'bg-zinc-800 text-violet-300 font-semibold' : 'text-zinc-400 hover:text-zinc-200'}`}
                        >
                            {chat.title}
                        </button>
                         <button onClick={() => deleteChat(chat.id)} className="p-1.5 text-zinc-600 hover:text-red-400 opacity-0 group-hover:opacity-100 transition-all" title="Delete Chat">
                            <Trash2 size={14} />
                        </button>
                    </div>
                ))}
...
const ChatAssetLibrary = () => {
    ...
    return (
        <div className="flex-shrink-0 mt-4 border-t border-zinc-800 pt-4">
            <h3 className="font-bold text-zinc-400 text-xs uppercase tracking-widest mb-3 px-1">Asset Library</h3>
            <div className="max-h-48 overflow-y-auto pr-1 custom-scrollbar">
                {chatAssets.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-6 border border-dashed border-zinc-800 rounded-lg opacity-40">
                         <ImageIcon size={20} className="mb-1" />
                         <p className="text-[10px] text-center px-4">Images from chats will appear here.</p>
                    </div>
                ) : (
                    <div className="grid grid-cols-3 gap-2">
                        {chatAssets.map((asset, index) => (
                            <div key={index} className="aspect-square bg-zinc-800 rounded-md overflow-hidden border border-white/5 shadow-inner">
...
const ChatSidebar: React.FC = () => {
    return (
        <aside className="w-72 flex-shrink-0 bg-zinc-925 border-r border-white/10 p-4 flex flex-col gap-4">
            <ChatHistoryList />
            <ChatAssetLibrary />
        </aside>
    );
...
export default function ChatStudio({ onBack }: { onBack: () => void }) {
    ...
    return (
        <div className="bg-zinc-950 text-zinc-200 h-screen w-full flex flex-col font-sans relative overflow-hidden">
             <header className="relative z-20 w-full flex justify-between items-center py-4 px-6 border-b border-white/10 flex-shrink-0 bg-zinc-950/50 backdrop-blur-xl">
                <button onClick={onBack} className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold transition-all bg-zinc-900 text-zinc-300 hover:bg-zinc-800 border border-zinc-800 shadow-inner-highlight">
                    <ArrowLeft size={16} /> Back to Tools
                </button>
            </header>
            <div className="flex flex-grow min-h-0 relative z-10">
                {activeTab === 'creative-chat' && <ChatSidebar />}
                <main className="flex-grow flex flex-col items-center p-6 min-w-0">
                    <div className="text-center mb-6 animate-fade-in">
                        <h2 className="text-3xl sm:text-4xl font-bold text-transparent bg-clip-text bg-gradient-to-br from-white to-zinc-400 mb-2 flex items-center justify-center gap-4 tracking-tighter italic uppercase font-black">
                            Chat Studio
                        </h2>
                        <p className="text-sm sm:text-base text-zinc-500 font-medium tracking-wide uppercase">Your creative co-pilot for prompt engineering and brainstorming.</p>
                    </div>
                     <div className="w-full max-w-lg mx-auto mb-8 animate-fade-in delay-100">
